...
const prodDependencies = (config) => {
  const prodDependencies = ['async', 'dotenv', 'nconf', 'winston'];
  if (config.framework && config.framework !== 'none') prodDependencies.push(config.framework);
  if (config.framework && config.framework === 'express') prodDependencies.push('cors', 'morgan', 'body-parser');
...
const devDependencies = (config) => {
  ...
  if (config.language === 'typescript') {
    ...
    if (config.framework && config.framework === 'express')
      devDependencies.push('@types/express', '@types/cors', '@types/morgan', '@types/body-parser');
  }
